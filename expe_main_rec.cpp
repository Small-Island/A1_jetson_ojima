...
void theta_record() {
    std::string cmd = "ptpcam --set-property=0x5013 --val=0x8002";
    printf("%s\n", cmd.c_str());
...
void start_a1_record() {
    ...
    a1MovingProfile.topVelocityDuration = 4;
    sendto(
        sockfd_send,
        &a1MovingProfile,
...
int main(int argc, char* argv[]) {
    ...
    cameraSetting();
    sleep(5);
    std::thread th_theta_record(theta_record);
    sleep(1);
    std::thread th_start_a1_record(start_a1_record);
    while (1) {
        sleep(10);
    }
    return 0;
